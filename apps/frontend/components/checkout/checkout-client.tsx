...
import { api, osLabelToId, regionLabelToId } from "@/lib/api-client"
...
export function CheckoutClient() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      const { order, checkout } = await api.createOrder({
        planId: plan.id,
        billingCycle: cycle,
        region: regionLabelToId(region),
        os: osLabelToId(osImage),
      })
      ...
      const rzp = new window.Razorpay({
        ...
        name: "IRCTC RDP",
        description: `${checkout.orderId} · ${plan.name}`,
        ...
        handler: async (response) => {
          try {
            ...
            // fall through to the success page; it polls the order status
          }
